...
## What does adaptive strategy routing mean?

It means the assistant is guided to choose the most useful reflective lens for the current session instead of using one fixed style. Depending on previous sessions and the user's current response, it may lean toward psychodynamic exploration, CBT-style tools, family-systems thinking, mindfulness, existential reflection, humanistic support, or safety-first boundaries.

The lens can change across sessions without losing continuity because the assistant still reads prior session notes, the running formulation, and the latest profile.

## Why separate session notes from psychological profile?

Session notes hold recent events and emotions. The psychological profile is one current working file that should be reviewed after every session. Stable patterns go into the main profile sections; uncertain clues stay provisional or become items to confirm.

## Why use one opening question?

One focused question keeps each session clear and prevents the assistant from overwhelming the user with too many directions.

## Can contributors add examples?

Yes, but examples must be fictional and clearly labeled as fictional.
